...
int8_t tmc2130_setup_chopper(uint8_t axis, uint8_t mres, uint8_t current_h, uint8_t current_r)
{
    ...
    if (current_r <= 31) {
        if (tmc2130_wr_CHOPCONF(axis, toff, hstrt, hend, fd3, 0, rndtf, chm, tbl, 1, 0, 0, 0, mres, intpol,
                                0, 0)) {
            return -1;
        }
        ...
        if (tmc2130_wr_CHOPCONF(axis, toff, hstrt, hend, fd3, 0, 0, 0, tbl, 0, 0, 0, 0, mres, intpol, 0,
                                0)) {
            return -1;
        }
        tmc2130_wr(axis, TMC2130_REG_IHOLD_IRUN,
